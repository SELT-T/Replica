...
export default function Sidebar({ onNavigate }) {
  const { user, canView, logout, isAdmin } = useAuth();
  const navigate = useNavigate();
  const [open, setOpen] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");

  // =========================================================
  // 🛑 SAFETY CHECK: सबसे पहले चेक करो कि यूजर है या नहीं
  // =========================================================
  if (!user) return null;
  ...
  // 1. FILTER: (Suraksha Kavach) - Check if canView exists to avoid crash
  const allowedItems = allItems.filter((item) => {
    // Agar AuthContext update nahi hua aur canView nahi mila, to sab dikha do (Crash mat karo)
    if (typeof canView !== "function") return true; 
    return canView(item.k);
  });
