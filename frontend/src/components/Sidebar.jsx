...
export default function Sidebar({ onNavigate, settings, isLight, t }) {
  ...
  // Colors adjusted to be more "Karbon" / Professional Blue-ish
  const allItems = [
    ...
    <>
      ...
      <button
        className={`fixed top-4 z-50 lg:hidden p-2 rounded-lg shadow-lg transition-transform active:scale-95 ${
          isLight ? "bg-white text-blue-900 border border-gray-100" : "bg-[#64FFDA] text-[#0A192F]"
      ...
      >
        {/* Header / Logo Section */}
        <div className={`flex flex-col items-center py-8 relative ${isLight ? "bg-gradient-to-b from-blue-50/30 to-transparent" : ""}`}>
          ...
          <p className={`text-[11px] font-medium uppercase tracking-widest mt-1 ${isLight ? "text-slate-500" : "text-gray-500"}`}>
          ...
          <div className={`mt-4 flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold border ${
             ...
             <div className={`w-2 h-2 rounded-full ${isLight ? "bg-green-600" : "bg-[#64FFDA] animate-pulse"}`}></div>
            ...
            filteredItems.map((it) => {
               ...
               if (isLight) {
                 itemClasses = isActive 
                   ? "bg-blue-50 text-blue-900 shadow-sm border border-blue-100" // Active: Blue-900 text on Blue-50 bg
        ...
        <div className={`absolute bottom-0 w-full p-4 border-t ${isLight ? "border-gray-100 bg-gray-50/50" : "border-[#1E2D45] bg-[#0A192F]"}`}>
           <p className={`text-[10px] text-center font-medium ${isLight ? "text-slate-400" : "text-gray-600"}`}>
