...
export default function Header({ onNavigate, openLogin, openSignup, isLight, currentLang, onLanguageChange, t, toggleSidebar }) {
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      if (menuRef.current && !menuRef.current.contains(event.target)) {
        // Only close if we are NOT clicking inside the popups
  ...
  const theme = {
    header: isLight 
      ? "bg-white/95 border-b border-gray-200 text-gray-800 backdrop-blur-xl shadow-sm" 
    ...
    textHighlight: isLight ? "text-blue-600" : "text-[#64FFDA]", // KARBONN BLUE PRESERVED
    statusColor: userStatus === "Active" ? "bg-green-500" : userStatus === "Busy" ? "bg-red-500" : "bg-yellow-500",
    popupBg: isLight ? "bg-white text-gray-800" : "bg-[#112240] text-white"
  };

  return (
    <>
      <header className={`sticky top-0 z-30 h-[70px] w-full transition-all duration-300 lg:pl-64 ${theme.header}`}>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
             ...
             <div className="relative">
                <button onClick={() => setShowLangMenu(!showLangMenu)} className={`p-2 rounded-full flex items-center gap-1 transition-all ${theme.iconBtn}`}>
                   <Globe size={18} />
                   <span className="text-xs font-bold hidden sm:inline">{currentLang === "English" ? "EN" : "HI"}</span>
                </button>
                {showLangMenu && (
             ...
             <div className="relative">
               ...
               {showNotifMenu && (
                 <div className={`popup-content absolute right-0 mt-4 w-80 rounded-2xl py-2 animate-in slide-in-from-top-2 border z-50 ${theme.dropdown}`}>
               ...
               <div className="relative ml-2">
                 ...
                 {showProfileMenu && (
                   <div className={`popup-content absolute right-0 mt-4 w-64 rounded-2xl py-2 animate-in slide-in-from-top-2 border z-50 ${theme.dropdown}`}>
                     ...
                     <div className="py-2">
                       {/* CLICKING MY PROFILE OPENS POPUP */}
      ...
      {/* --- PROFILE POPUP (NEW FEATURE) --- */}
      {showProfilePopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in">
          <div className={`w-full max-w-md mx-4 rounded-2xl shadow-2xl border relative overflow-hidden ${theme.dropdown}`}>
