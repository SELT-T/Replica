...
export default function Reports() {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] text-slate-800 p-2 sm:p-4 font-sans transition-colors duration-300 overflow-x-hidden">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4 sm:mb-6">
        ...
        <div className="flex flex-wrap gap-2 w-full md:w-auto justify-center md:justify-end mt-2 md:mt-0">
           <div className="bg-white px-2 py-1.5 sm:px-3 sm:py-2 rounded-xl shadow-sm border border-blue-100 flex items-center gap-2 min-w-[100px] sm:min-w-[120px]">
              <div className="p-1 bg-blue-100 rounded-lg text-blue-600 font-bold text-xs">REC</div>
              <div>
           ...
           <div className="bg-white px-2 py-1.5 sm:px-3 sm:py-2 rounded-xl shadow-sm border border-green-100 flex items-center gap-2 min-w-[100px] sm:min-w-[120px]">
              <div className="p-1 bg-green-100 rounded-lg text-green-600 font-bold text-xs">QTY</div>
              <div>
           ...
           <div className="bg-white px-2 py-1.5 sm:px-3 sm:py-2 rounded-xl shadow-sm border border-purple-100 flex items-center gap-2 min-w-[100px] sm:min-w-[120px]">
              <div className="p-1 bg-purple-100 rounded-lg text-purple-600 font-bold text-xs">AMT</div>
              <div>
      ...
      <div className="bg-white p-2 sm:p-3 rounded-xl shadow-md border border-gray-100 mb-4 sm:mb-6 flex gap-2 overflow-x-auto">
        <button 
          ...
          className={`px-3 py-2 rounded-lg font-bold text-xs transition-all flex-shrink-0 ${viewMode === "table" ? "bg-blue-600 text-white shadow-md" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
        >
          📊 Table View
        </button>
        <button 
          onClick={() => { setViewMode("statement"); setPage(1); }}
          className={`px-3 py-2 rounded-lg font-bold text-xs transition-all flex-shrink-0 ${viewMode === "statement" ? "bg-blue-600 text-white shadow-md" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
        >
          📈 Statement View
        </button>
      </div>
      ...
      <div className="bg-white p-3 sm:p-4 rounded-2xl shadow-md border border-gray-100 mb-4 sm:mb-6 space-y-4">
         
         <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-between items-center">
            <div className="flex gap-2 w-full sm:w-auto overflow-x-auto pb-1">
                <button onClick={loadData} className="flex items-center gap-1 sm:gap-2 px-3 py-2 rounded-lg bg-gray-100 text-gray-700 font-bold text-xs hover:bg-gray-200 transition-colors shadow-sm flex-shrink-0">
                   <span>🔄</span> <span className="hidden sm:inline">Refresh</span>
                </button>
                <button onClick={viewMode === "table" ? exportExcel : exportStatementExcel} className="flex items-center gap-1 sm:gap-2 px-3 py-2 rounded-lg bg-green-50 text-green-700 font-bold text-xs hover:bg-green-100 transition-colors border border-green-200 shadow-sm flex-shrink-0">
                   <span>📊</span> <span className="hidden sm:inline">Excel</span>
                </button>
                <button onClick={viewMode === "table" ? exportPDF : exportStatementPDF} className="flex items-center gap-1 sm:gap-2 px-3 py-2 rounded-lg bg-red-50 text-red-700 font-bold text-xs hover:bg-red-100 transition-colors border border-red-200 shadow-sm flex-shrink-0">
                   <span>📄</span> <span className="hidden sm:inline">PDF</span>
                </button>
                {viewMode === "table" && (
                  <button onClick={() => setExcelOpen(true)} className="flex items-center gap-1 sm:gap-2 px-3 py-2 rounded-lg bg-indigo-50 text-indigo-700 font-bold text-xs hover:bg-indigo-100 transition-colors border border-indigo-200 shadow-sm flex-shrink-0">
                     <span>👁️</span> <span className="hidden sm:inline">View</span>
                  </button>
                )}
            </div>
         ...
         <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
            ...
            {dateRange === "Custom" && (
               <div className="col-span-1 xs:col-span-2 flex flex-col xs:flex-row gap-2 items-end">
      ...
      {/* TABLE VIEW - FIXED: लास्ट रो बाहर नहीं जाएगी */}
      {viewMode === "table" && (
        <div className="rounded-xl border border-gray-200 bg-white shadow-xl overflow-hidden flex flex-col">
                    ...
                    <tr>
                      {DISPLAY_COLUMNS.map((col) => (
                        <th 
                            ...
                            className="px-2 sm:px-3 py-2 sm:py-3 font-bold uppercase tracking-wider whitespace-nowrap cursor-pointer hover:bg-white/10 transition-colors select-none"
                        >
                      ...
                      pageRows.map((row, idx) => {
                        ...
                        return (
                          <tr key={idx} className="hover:bg-blue-50 transition-colors even:bg-slate-50/50 group">
                            <td className="px-2 sm:px-3 py-1.5 sm:py-2.5 text-center text-gray-400 font-mono border-r border-gray-100">{row["Sr.No"]}</td>
                            <td className="px-2 sm:px-3 py-1.5 sm:py-2.5 text-gray-600 border-r border-gray-100 whitespace-nowrap">{row.Date}</td>
                            <td className="px-2 sm:px-3 py-1.5 sm:py-2.5 font-bold text-slate-700 border-r border-gray-100 truncate max-w-[120px] sm:max-w-none">{row["Party Name"]}</td>
                            <td className="px-2 sm:px-3 py-1.5 sm:py-2.5 text-gray-600 border-r border-gray-100 truncate max-w-[100px] sm:max-w-none">{row["Item Name"]}</td>
                            <td className="px-2 sm:px-3 py-1.5 sm:py-2.5 text-gray-500 border-r border-gray-100 truncate max-w-[100px] sm:max-w-none">{row["Item Category"]}</td>
                            <td className="px-2 sm:px-3 py-1.5 sm:py-2.5 text-gray-500 border-r border-gray-100 truncate max-w-[80px] sm:max-w-none">{row["City/Area"]}</td>
                            <td className="px-2 sm:px-3 py-1.5 sm:py-2.5 text-indigo-600 font-medium border-r border-gray-100 truncate max-w-[100px] sm:max-w-none">{row["Item Group"]}</td>
                            <td className="px-2 sm:px-3 py-1.5 sm:py-2.5 text-orange-600 font-medium border-r border-gray-100 truncate max-w-[100px] sm:max-w-none">{row["Salesman"]}</td>
                            <td className="px-2 sm:px-3 py-1.5 sm:py-2.5 text-right font-mono text-gray-700 border-r border-gray-100">{row.Qty}</td>
                            <td className="px-2 sm:px-3 py-1.5 sm:py-2.5 text-right font-bold text-blue-600 font-mono border-r border-gray-100">₹{row.Amount.toLocaleString("en-IN")}</td>
                            <td className="px-2 sm:px-3 py-1.5 sm:py-2.5 text-right font-bold text-emerald-600 font-mono">{percent}</td>
                          </tr>
                        );
                      })
                    )}
                    
                    {/* TOTALS ROW - FIXED: यह अब बाहर नहीं जाएगी */}
                    {!loading && filtered.length > 0 && (
                      <tr className="bg-gradient-to-r from-blue-50 to-indigo-50 border-t-2 border-blue-300 font-bold text-slate-800">
                        <td colSpan="8" className="px-2 sm:px-3 py-2 sm:py-3 text-right uppercase text-xs">TOTAL:</td>
                        <td className="px-2 sm:px-3 py-2 sm:py-3 text-right font-mono text-blue-700">{totalQty.toLocaleString("en-IN")}</td>
                        <td className="px-2 sm:px-3 py-2 sm:py-3 text-right font-mono text-blue-700">₹{totalAmount.toLocaleString("en-IN")}</td>
                        <td className="px-2 sm:px-3 py-2 sm:py-3 text-right font-mono text-emerald-600">100.00%</td>
                      </tr>
                    )}
                  </tbody>
                </table>
          ...
          {/* Pagination Footer */}
          <div className="bg-white border-t border-gray-200 p-2 sm:p-3 flex flex-col sm:flex-row justify-between items-center gap-2 text-xs">
             ...
             <div className="flex gap-2">
                <button disabled={page === 1} onClick={() => setPage(page - 1)} className="px-3 sm:px-4 py-1.5 rounded-lg border border-gray-300 text-gray-600 font-bold hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition-all">Previous</button>
                <button disabled={page === totalPages || totalPages === 0} onClick={() => setPage(page + 1)} className="px-3 sm:px-4 py-1.5 rounded-lg bg-blue-600 text-white font-bold hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-md transition-all">Next</button>
             </div>
          </div>
        </div>
      )}
            ...
            <div className="min-w-full sm:min-w-[600px]">
              <table className="w-full text-xs text-left border-collapse">
                <thead className="bg-gradient-to-r from-green-600 to-emerald-700 text-white sticky top-0 z-10 shadow-md">
                  <tr>
                    <th className="px-2 sm:px-4 py-2 sm:py-3 font-bold uppercase tracking-wider">Sr.No</th>
                    <th className="px-2 sm:px-4 py-2 sm:py-3 font-bold uppercase tracking-wider">
                    ...
                    <th className="px-2 sm:px-4 py-2 sm:py-3 font-bold uppercase tracking-wider text-right">Quantity</th>
                    <th className="px-2 sm:px-4 py-2 sm:py-3 font-bold uppercase tracking-wider text-right">Amount</th>
                    <th className="px-2 sm:px-4 py-2 sm:py-3 font-bold uppercase tracking-wider text-right">No. of Items</th>
                    <th className="px-2 sm:px-4 py-2 sm:py-3 font-bold uppercase tracking-wider text-right">% of Total</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {loading ? (
                    ...
                    statementData.map((row, idx) => (
                      <tr key={idx} className="hover:bg-green-50 transition-colors even:bg-slate-50/50">
                        <td className="px-2 sm:px-4 py-1.5 sm:py-3 text-center text-gray-400 font-mono border-r border-gray-100">{idx + 1}</td>
                        <td className="px-2 sm:px-4 py-1.5 sm:py-3 font-bold text-slate-700 border-r border-gray-100 truncate max-w-[150px] sm:max-w-none">{row.name}</td>
                        <td className="px-2 sm:px-4 py-1.5 sm:py-3 text-right font-mono text-gray-700">{row.qty.toLocaleString("en-IN")}</td>
                        <td className="px-2 sm:px-4 py-1.5 sm:py-3 text-right font-bold text-blue-600 font-mono">₹{row.amount.toLocaleString("en-IN")}</td>
                        <td className="px-2 sm:px-4 py-1.5 sm:py-3 text-right font-mono text-gray-600">{row.items}</td>
                        <td className="px-2 sm:px-4 py-1.5 sm:py-3 text-right font-bold text-emerald-600 font-mono">{row.percentage}%</td>
                      </tr>
                    ))
                  )}
                  ...
                  {!loading && statementData.length > 0 && (
                    <tr className="bg-gradient-to-r from-green-50 to-emerald-50 border-t-2 border-green-300 font-bold text-slate-800">
                      <td colSpan="2" className="px-2 sm:px-4 py-1.5 sm:py-3 text-right uppercase text-xs">TOTAL:</td>
                      <td className="px-2 sm:px-4 py-1.5 sm:py-3 text-right font-mono text-green-700">{totalQty.toLocaleString("en-IN")}</td>
                      <td className="px-2 sm:px-4 py-1.5 sm:py-3 text-right font-mono text-green-700">₹{totalAmount.toLocaleString("en-IN")}</td>
                      <td className="px-2 sm:px-4 py-1.5 sm:py-3 text-right font-mono text-green-700">{statementData.reduce((a, b) => a + b.items, 0)}</td>
                      <td className="px-2 sm:px-4 py-1.5 sm:py-3 text-right font-mono text-emerald-600">100.00%</td>
                    </tr>
                  )}
                </tbody>
              </table>
      ...
      {/* EXCEL PREVIEW MODAL */}
      {excelOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex justify-center items-center p-1 sm:p-4 z-50">
          <div className="bg-white w-full max-w-7xl h-[85vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-fadeIn mx-2">
            <div className="p-3 sm:p-4 bg-gray-50 border-b flex justify-between items-center">
              ...
              <button onClick={() => setExcelOpen(false)} className="bg-red-50 text-red-500 w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center font-bold hover:bg-red-500 hover:text-white transition-all">✕</button>
            </div>
            <div className="flex-1 overflow-auto p-1 sm:p-4 bg-white">
              <div className="overflow-x-auto">
                <table className="min-w-full border-collapse border border-gray-300 text-xs">
                  <thead>
                    <tr className="bg-gray-100">
                      {DISPLAY_COLUMNS.map(c => <th key={c} className="border border-gray-300 px-2 sm:px-3 py-2 text-left text-gray-600 uppercase font-bold whitespace-nowrap">{c}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                    {filtered.slice(0, 100).map((row, i) => {
                      ...
                      return (
                        <tr key={i} className="hover:bg-blue-50">
                          <td className="border border-gray-300 px-2 sm:px-3 py-1 sm:py-1.5 text-center">{row["Sr.No"]}</td>
                          <td className="border border-gray-300 px-2 sm:px-3 py-1 sm:py-1.5">{row["Date"]}</td>
                          <td className="border border-gray-300 px-2 sm:px-3 py-1 sm:py-1.5 font-medium truncate max-w-[100px] sm:max-w-none">{row["Party Name"]}</td>
                          <td className="border border-gray-300 px-2 sm:px-3 py-1 sm:py-1.5 truncate max-w-[80px] sm:max-w-none">{row["Item Name"]}</td>
                          <td className="border border-gray-300 px-2 sm:px-3 py-1 sm:py-1.5 truncate max-w-[80px] sm:max-w-none">{row["Item Category"]}</td>
                          <td className="border border-gray-300 px-2 sm:px-3 py-1 sm:py-1.5 truncate max-w-[60px] sm:max-w-none">{row["City/Area"]}</td>
                          <td className="border border-gray-300 px-2 sm:px-3 py-1 sm:py-1.5 truncate max-w-[80px] sm:max-w-none">{row["Item Group"]}</td>
                          <td className="border border-gray-300 px-2 sm:px-3 py-1 sm:py-1.5 truncate max-w-[80px] sm:max-w-none">{row["Salesman"]}</td>
                          <td className="border border-gray-300 px-2 sm:px-3 py-1 sm:py-1.5 text-right">{row["Qty"]}</td>
                          <td className="border border-gray-300 px-2 sm:px-3 py-1 sm:py-1.5 text-right font-bold">₹{row["Amount"].toLocaleString("en-IN")}</td>
                          <td className="border border-gray-300 px-2 sm:px-3 py-1 sm:py-1.5 text-right">{percent}</td>
                        </tr>
                      );
                    })}
                  </tbody>
              ...
              {filtered.length > 100 && <p className="text-center mt-3 text-gray-500 italic text-xs sm:text-sm">Preview limited to first 100 rows. Download Excel for full data.</p>}
            </div>
          </div>
        </div>
      )}
