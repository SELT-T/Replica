    ...
    try {
      ...
      if (json.success && json.data) {
        const mapped = json.data.map((row, i) => ({
          ...
          // Fixed: Prioritize item_category, fallback to parent or group if missing
          "Item Category": row.item_category || row.parent || row.category || "N/A",
    ...
    <div className="min-h-screen bg-[#F8F9FA] text-slate-800 p-2 md:p-4 font-sans transition-colors duration-300">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        ...
        <div className="flex flex-wrap gap-2 md:gap-3 w-full md:w-auto">
           <div className="flex-1 min-w-[100px] md:min-w-0 bg-white px-3 py-2 md:px-4 md:py-2 rounded-xl shadow-sm border border-blue-100 flex items-center gap-2 md:gap-3">
              <div className="p-1.5 bg-blue-100 rounded-lg text-blue-600 font-bold text-xs hidden md:block">REC</div>
              <div>
           ...
           <div className="flex-1 min-w-[100px] md:min-w-0 bg-white px-3 py-2 md:px-4 md:py-2 rounded-xl shadow-sm border border-green-100 flex items-center gap-2 md:gap-3">
              <div className="p-1.5 bg-green-100 rounded-lg text-green-600 font-bold text-xs hidden md:block">QTY</div>
              <div>
           ...
           <div className="flex-1 min-w-[120px] md:min-w-0 bg-white px-3 py-2 md:px-4 md:py-2 rounded-xl shadow-sm border border-purple-100 flex items-center gap-2 md:gap-3">
              <div className="p-1.5 bg-purple-100 rounded-lg text-purple-600 font-bold text-xs hidden md:block">AMT</div>
              <div>
      ...
      {viewMode === "table" && (
        <div className="rounded-xl border border-gray-200 bg-white shadow-xl overflow-hidden flex flex-col" style={{ height: "calc(100vh - 380px)" }}>
          <div className="flex-1 overflow-x-auto overflow-y-auto">
              <table className="w-full text-xs text-left border-collapse min-w-[1200px] md:min-w-full">
                  ...
                  <tr>
                    {DISPLAY_COLUMNS.map((col) => (
                      <th 
                          ...
                          className="px-4 py-3 font-bold uppercase tracking-wider whitespace-nowrap cursor-pointer hover:bg-white/10 transition-colors select-none first:pl-6 last:pr-6"
                          style={{ minWidth: col === "Item Name" ? "200px" : col === "Party Name" ? "180px" : "100px" }}
                      >
                ...
                <tbody className="divide-y divide-gray-100">
                    ...
                    pageRows.map((row, idx) => {
                      ...
                      return (
                        <tr key={idx} className="hover:bg-blue-50 transition-colors even:bg-slate-50/50 group">
                          <td className="px-4 py-2.5 text-center text-gray-400 font-mono border-r border-gray-100 first:pl-6 whitespace-nowrap">{row["Sr.No"]}</td>
                          <td className="px-4 py-2.5 text-gray-600 border-r border-gray-100 whitespace-nowrap">{row.Date}</td>
                          <td className="px-4 py-2.5 font-bold text-slate-700 border-r border-gray-100 whitespace-nowrap max-w-[200px] overflow-hidden text-ellipsis" title={row["Party Name"]}>{row["Party Name"]}</td>
                          <td className="px-4 py-2.5 text-gray-600 border-r border-gray-100 whitespace-nowrap max-w-[250px] overflow-hidden text-ellipsis" title={row["Item Name"]}>{row["Item Name"]}</td>
                          ...
                          <td className="px-4 py-2.5 text-right font-mono text-gray-700 border-r border-gray-100 whitespace-nowrap">{row.Qty}</td>
                          <td className="px-4 py-2.5 text-right font-bold text-blue-600 font-mono border-r border-gray-100 whitespace-nowrap">₹{row.Amount.toLocaleString("en-IN")}</td>
                          <td className="px-4 py-2.5 text-right font-bold text-emerald-600 font-mono last:pr-6 whitespace-nowrap">{percent}</td>
                        </tr>
                      );
                    })
                  )}
                  ...
                  {!loading && filtered.length > 0 && (
                    <tr className="bg-gradient-to-r from-blue-50 to-indigo-50 border-t-2 border-blue-300 font-bold text-slate-800 sticky bottom-0 z-10">
                      <td colSpan="8" className="px-4 py-3 text-right uppercase text-xs first:pl-6">TOTAL:</td>
                      <td className="px-4 py-3 text-right font-mono text-blue-700 whitespace-nowrap">{totalQty.toLocaleString("en-IN")}</td>
                      <td className="px-4 py-3 text-right font-mono text-blue-700 whitespace-nowrap">₹{totalAmount.toLocaleString("en-IN")}</td>
                      <td className="px-4 py-3 text-right font-mono text-emerald-600 last:pr-6 whitespace-nowrap">100.00%</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          {/* Pagination Footer */}
          <div className="bg-white border-t border-gray-200 p-3 flex flex-col md:flex-row gap-3 justify-between items-center text-xs">
                  ...
                  {filtered.slice(0, 100).map((row, i) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-blue-50">
                        ...
                        <td className="border border-gray-300 px-3 py-1.5 text-right whitespace-nowrap">{row["Qty"]}</td>
                        <td className="border border-gray-300 px-3 py-1.5 text-right font-bold whitespace-nowrap">{row["Amount"]}</td>
                        <td className="border border-gray-300 px-3 py-1.5 text-right whitespace-nowrap">{percent}</td>
                      </tr>
                    );
                  })}
                </tbody>
