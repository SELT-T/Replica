...
// PROPS: isLight (Theme), t (Language), openLogin/openSignup (Buttons)
export default function Dashboard({ isLight, t = (s) => s, openLogin, openSignup }) {
  ...
  // --- DYNAMIC THEME COLORS (Based on isLight prop) ---
  const colors = {
    bg: isLight ? "bg-[#F0F2F5]" : "bg-gradient-to-br from-[#0A192F] via-[#112240] to-[#0A192F]",
    card: isLight ? "bg-white border-gray-300 shadow-md" : "bg-[#1B2A4A] border-[#1E2D45] shadow-xl",
    innerCard: isLight ? "bg-gray-50 border-gray-200" : "bg-[#0F1E33] border-[#1E2D45]", // For Charts/Stats
    textMain: isLight ? "text-[#0A192F]" : "text-gray-100",
    textMuted: isLight ? "text-gray-500" : "text-gray-400",
    accentText: isLight ? "text-[#0A192F]" : "text-[#64FFDA]", // Carbon Blue in Light, Teal in Dark
...
const colValue = (r, col) => {
  ...
  return (
    <div className={`min-h-screen ${colors.bg} ${colors.textMain} p-2 sm:p-4 md:p-6 transition-colors duration-300`}>
      <div className={`max-w-[1450px] mx-auto ${colors.card} rounded-2xl p-4 md:p-6 space-y-6 border ${colors.border}`}>
        ...
        {activeTab === "overview" && (
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              <StatBox title={t('Total Sales')} value={fmt(totalSales)} sub={`${cleanData.length} trans`} colors={colors} />
              <StatBox title={t('Parties')} value={new Set(cleanData.map(r => r["Party Name"]).filter(v => v && v !== 'N/A')).size} sub="Customers" colors={colors} />
              <StatBox title={t('Vouchers')} value={uniqueVoucherNumbers} sub="Bills" colors={colors} />
              <StatBox title={t('Products')} value={totalProducts} sub="Items" colors={colors} />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
              ...
              {(() => {
                ...
                return (
                  <div className={`${colors.innerCard} border rounded-xl p-4 shadow-lg h-[260px] overflow-hidden`}>
        ...
        {activeTab === "performers" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            ...
            {(() => {
              ...
              return (
                <div className={`${colors.innerCard} rounded-lg p-3 border shadow-lg`}>
...
// Sub-components
function StatBox({ title, value, sub, colors }) {
  return (
    <div className={`${colors.innerCard} border rounded-xl p-4 shadow-lg hover:shadow-2xl hover:scale-[1.02] transition-all duration-300`}>
      <p className={`text-[10px] sm:text-xs opacity-90 ${colors.textMuted}`}>{title}</p>
      <h3 className={`text-sm sm:text-xl md:text-2xl font-bold mt-1 ${colors.textMain}`}>{value}</h3>
      <p className={`text-[8px] sm:text-[10px] opacity-75 mt-1 ${colors.textMuted}`}>{sub}</p>
    </div>
  );
}
...
function ReportCard({ title, columns, data, onView, onRowClick, filter1Value, filter1Options, onFilter1Change, filter1Label, colors, t }) {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter(row => columns.some(col => String(row[col] || "").toLowerCase().includes(searchTerm.toLowerCase())));
  ...
  return (
    <div className={`${colors.innerCard} rounded-lg p-3 shadow-lg border`}>
      <div className={`flex justify-between items-center mb-2 border-b pb-2 ${colors.border}`}>
        <h4 className={`font-bold text-xs sm:text-sm ${colors.accentText}`}>{title}</h4>
        <div className="flex gap-1">
          <button onClick={onView} className="bg-rose-500 text-white text-[9px] sm:text-[10px] px-1.5 sm:px-2 py-1 rounded hover:bg-rose-600">{t('View')}</button>
        </div>
      </div>
      <div className="mb-2">
        <input type="text" placeholder="🔍 Search..." className={`w-full border rounded px-2 py-1.5 text-[10px] sm:text-xs focus:outline-none focus:ring-1 focus:ring-[#64FFDA] ${colors.inputBg}`} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
      </div>
      <div className="flex gap-1 mb-2">
        <select value={filter1Value} onChange={(e) => onFilter1Change(e.target.value)} className={`flex-1 border rounded px-1.5 py-1 text-[10px] sm:text-xs focus:outline-none focus:ring-1 focus:ring-[#64FFDA] ${colors.inputBg}`}>
          <option value="">All {filter1Label}</option>{filter1Options.map((opt, i) => <option key={i} value={opt}>{opt}</option>)}
        </select>
        {filter1Value && <button onClick={() => onFilter1Change("")} className="bg-red-500 text-white text-[10px] px-2 py-1 rounded hover:bg-red-600">×</button>}
      </div>
      <div className={`overflow-auto max-h-[220px] border rounded ${colors.border}`}>
        <table className="w-full text-[9px] sm:text-[10px]">
          <thead className={`sticky top-0 z-10 ${colors.card.includes("white") ? "bg-gray-200 text-[#0A192F]" : "bg-[#0B2545] text-[#64FFDA]"}`}>
            <tr>{columns.map((c, i) => <th key={i} className={`px-1.5 sm:px-2 py-1.5 text-left font-semibold ${i === columns.length - 1 ? "text-right" : ""}`}>{c}</th>)}</tr>
          </thead>
          <tbody>
            {filteredData.length === 0 && <tr><td colSpan={columns.length} className={`text-center py-3 text-[10px] ${colors.textMuted}`}>No Data</td></tr>}
            {filteredData.slice(0, 20).map((row, i) => (
              <tr key={i} onClick={() => onRowClick && onRowClick(row)} className={`hover:opacity-80 cursor-pointer border-b ${colors.card.includes("white") ? "hover:bg-gray-100 border-gray-200" : "hover:bg-[#1C3F57] border-[#1E2D45]"}`}>
                {columns.map((c, j) => (
                  <td key={j} className={`px-1.5 sm:px-2 py-1.5 ${j === columns.length - 1 ? `text-right font-semibold ${colors.accentText}` : ""}`}>{c === "Amount" ? fmt(row[c]) : c === "Qty" ? row[c]?.toLocaleString("en-IN") : row[c] || "-"}</td>
                ))}
              </tr>
            ))}
            {filteredData.length > 0 && (
              <tr className={`font-bold border-t-2 sticky bottom-0 z-20 shadow-lg ${colors.card.includes("white") ? "bg-gray-100 border-yellow-500 text-[#0A192F]" : "bg-[#0F1E33] border-yellow-400 text-yellow-300"}`}>
